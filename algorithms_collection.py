...
class FilterBankTangentSpace:
    ...
    def extract_features(self, X):
        """Extract features from EEG data for visualization.
    ...
    def compute_ea_whitening_matrix(self, X_rest):
        """Compute Euclidean Alignment (EA) whitening matrix from resting-state data.
        
        EA aligns different sessions by whitening the data to a common reference point
        (identity matrix), which helps reduce session-to-session variability caused by
        electrode position changes.
        
        Args:
            X_rest: Resting-state EEG data of shape (n_samples, n_channels, n_times)
                   or (n_channels, n_times) for single trial
            
        Returns:
            whitening_matrix: EA whitening matrix of shape (n_channels, n_channels)
        """
        # Handle single trial case
        if X_rest.ndim == 2:
            X_rest = X_rest[np.newaxis, :, :]
        
        # Compute average covariance matrix across all resting trials
        cov_matrices = []
        for trial in X_rest:
            # Use OAS estimator for robust covariance estimation
            cov = Covariances(estimator='oas').fit_transform(trial[np.newaxis, :, :])[0]
            cov_matrices.append(cov)
        
        mean_cov = np.mean(cov_matrices, axis=0)
        
        # Compute whitening matrix: R^(-1/2)
        # Eigenvalue decomposition: R = U @ Lambda @ U.T
        eigenvalues, eigenvectors = np.linalg.eigh(mean_cov)
        
        # Ensure positive definiteness
        eigenvalues = np.maximum(eigenvalues, 1e-10)
        
        # Whitening matrix: U @ Lambda^(-1/2) @ U.T
        whitening_matrix = eigenvectors @ np.diag(1.0 / np.sqrt(eigenvalues)) @ eigenvectors.T
        
        return whitening_matrix
    
    def apply_ea_alignment(self, X, whitening_matrix):
        """Apply Euclidean Alignment to EEG data.
        
        Args:
            X: EEG data of shape (n_samples, n_channels, n_times)
            whitening_matrix: EA whitening matrix of shape (n_channels, n_channels)
            
        Returns:
            X_aligned: Aligned EEG data of shape (n_samples, n_channels, n_times)
        """
        X_aligned = np.zeros_like(X)
        for i in range(X.shape[0]):
            X_aligned[i] = whitening_matrix @ X[i]
        return X_aligned
