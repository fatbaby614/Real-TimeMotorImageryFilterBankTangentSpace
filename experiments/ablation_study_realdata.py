...
class AblationStudyRealData:
    """Perform ablation study with real MAT file data."""
    ...
    def create_config_variants(self) -> Dict[str, FBCSPConfig]:
        ...
        # 4. Classic CSP (single band + no Riemann)
        ...
        # 8. Only low beta
        variants['LowBeta_Only'] = FBCSPConfig(
            **base_config,
            filter_banks=[(10, 16)],
            use_riemann_tangent=True
        )
        ...
        # 9. Only high beta
        variants['HighBeta_Only'] = FBCSPConfig(
            **base_config,
            filter_banks=[(24, 32)],
            use_riemann_tangent=True
        )
    ...
    def run_study(self) -> pd.DataFrame:
        ...
        print("=" * 60)
        print("Ablation Study with Real Data")
        print("=" * 60)
    ...
    def save_results(self, output_path: str = None):
        ...
        if output_path is None:
            output_path = get_results_path(get_timestamped_filename('ablation_results_realdata', 'csv'))
        else:
    ...
    def plot_results(self, save_path: str = None):
        ...
        try:
            ...
            # Highlight classic CSP
            if 'Classic_CSP' in configs:
                csp_idx = configs.index('Classic_CSP')
                bars[csp_idx].set_color('coral')
                bars[csp_idx].set_alpha(0.9)
            
            ax.set_xlabel('Configuration', fontsize=12)
            ax.set_ylabel('Accuracy', fontsize=12)
            ax.set_title('Ablation Study: Component Contributions (Real Data)', fontsize=14)
            ax.set_xticks(x_pos)
...
def load_mat_data(mat_path: str) -> Tuple[np.ndarray, np.ndarray]:
    ...
    y = np.asarray(mat["labels"], dtype=np.int32)
    
    print(f"Loaded {X.shape[0]} trials, {len(np.unique(y))} classes")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Ablation study with real MAT data")
    ...
    # Load data
    X, y = load_mat_data(args.mat_file)
    
    # Run study
    study = AblationStudyRealData(X, y, n_splits=args.n_splits)
    df = study.run_study()
    study.print_results()
    ...
    if args.output:
        ...
        study.save_results()
        # Save plot with timestamp
        plot_path = get_results_path(get_timestamped_filename('ablation_results_realdata', 'png'))
